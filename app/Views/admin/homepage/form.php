...
<form class="content-form" method="post" enctype="multipart/form-data" action="<?= View::escape($baseUrl) ?>admin/homepage<?= $editing ? '/' . (int) $itemId : '' ?>">
    ...
    <section class="form-section">
        ...
        <div class="form-grid">
            <div class="form-field span-2"><label for="media_id">Existing desktop image</label><select id="media_id" name="media_id"><option value="">Choose an image</option><?php foreach ($images as $image): ?><option value="<?= (int) $image['id'] ?>" <?= $value('media_id') === (string) $image['id'] ? 'selected' : '' ?>><?= View::escape($image['original_name']) ?> (<?= (int) $image['width'] ?>×<?= (int) $image['height'] ?>)</option><?php endforeach; ?></select></div>
            ...
            <div class="form-field span-2"><label for="mobile_media_id">Optional mobile image</label><select id="mobile_media_id" name="mobile_media_id"><option value="">Use desktop image</option><?php foreach ($images as $image): ?><option value="<?= (int) $image['id'] ?>" <?= $value('mobile_media_id') === (string) $image['id'] ? 'selected' : '' ?>><?= View::escape($image['original_name']) ?></option><?php endforeach; ?></select></div>
        </div>
    </section>
