...
<form class="content-form" method="post" action="<?= View::escape($baseUrl) ?>admin/pages<?= $editing ? '/' . $itemId : '' ?>">
    ...
    <section class="form-section">
        <div class="form-grid">
            ...
            <div class="form-field span-2"><label for="page-hero-image">Masthead image</label><select id="page-hero-image" name="hero_media_id"><option value="">Use the default FAST building image</option><?php foreach ($images as $image): ?><option value="<?= (int) $image['id'] ?>" <?= $value('hero_media_id') === (string) $image['id'] ? 'selected' : '' ?>><?= View::escape($image['original_name']) ?></option><?php endforeach; ?></select><small>This image appears beside the page title. Upload new photographs in the Media library first.</small><?php if ($value('hero_path') !== ''): ?><small>Current image: <?= View::escape(basename($value('hero_path'))) ?></small><?php endif; ?></div>
        </div>
    </section>
        ...
        <details class="page-section-add" <?= $sections === [] ? 'open' : '' ?>>
            ...
            <form class="content-form" method="post" action="<?= View::escape($baseUrl) ?>admin/pages/<?= $itemId ?>/sections">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-field span-2"><label>Image</label><select name="media_id"><option value="">No image</option><?php foreach ($images as $image): ?><option value="<?= (int) $image['id'] ?>"><?= View::escape($image['original_name']) ?></option><?php endforeach; ?></select><small>For multiple History images, add several Gallery image sections.</small></div>
