...
final class SiteContentValidator
{
    ...
    {
        $errors=[];$title=$this->text($input['title']??'',255);$slug=$this->slug((string)($input['slug']??''),$title);$parent=$this->id($input['parent_page_id']??null);$hero=$this->id($input['hero_media_id']??null);if($title==='')$errors[]='Page title is required.';if($slug===''||$this->repository->slugExists('pages',$slug,$id))$errors[]=$slug===''?'A valid page URL is required.':'That page URL is already in use.';if($parent!==null&&($parent===$id||!$this->repository->relation('pages',$parent)))$errors[]='Choose a valid parent page.';if($hero!==null&&!$this->repository->relation('media',$hero))$errors[]='Choose an active masthead image.';return ['data'=>['parent_page_id'=>$parent,'title'=>$title,'slug'=>$slug,'page_template'=>$this->text($input['page_template']??'standard',100)?:'standard','meta_title'=>$this->nullable($input['meta_title']??null,255),'meta_description'=>$this->nullable($input['meta_description']??null,320),'hero_media_id'=>$hero,'display_order'=>max(0,min(65535,(int)($input['display_order']??0))),'show_in_navigation'=>$this->flag($input['show_in_navigation']??null)],'errors'=>$errors];
    }

    /** @param array<string,mixed> $input @return array{data:array<string,mixed>,errors:list<string>} */
    public function section(array $input): array
    {
        $types=['rich_text','image_text','cards','statistics','call_to_action','quote','gallery','video','custom'];$type=(string)($input['section_type']??'rich_text');$media=$this->id($input['media_id']??null);$errors=[];if(!in_array($type,$types,true))$errors[]='Choose a valid section type.';if($media!==null&&!$this->repository->relation('media',$media))$errors[]='Choose an active section image.';$url=$this->nullable($input['button_url']??null,500);if($url!==null&&!str_starts_with($url,'/')&&!filter_var($url,FILTER_VALIDATE_URL))$errors[]='Enter a complete button URL or a path beginning with /.';
        $extraIds=[];
        foreach(is_array($input['extra_media_id']??null)?$input['extra_media_id']:[] as$value){
            $extraId=$this->id($value);
            if($extraId===null)continue;
            if(!$this->repository->relation('media',$extraId)){$errors[]='Choose valid additional photos.';continue;}
            $extraIds[]=$extraId;
        }
        $extraIds=array_slice(array_values(array_unique($extraIds)),0,2);
        $settingsJson=$extraIds!==[]?json_encode(['extra_media_ids'=>$extraIds],JSON_THROW_ON_ERROR):null;
        return ['data'=>['section_type'=>$type,'heading'=>$this->nullable($input['heading']??null,255),'subheading'=>$this->nullable($input['subheading']??null,255),'body'=>$this->nullable($input['body']??null,150000),'media_id'=>$media,'button_label'=>$this->nullable($input['button_label']??null,100),'button_url'=>$url,'settings_json'=>$settingsJson,'display_order'=>max(0,min(65535,(int)($input['display_order']??0))),'is_visible'=>$this->flag($input['is_visible']??null)],'errors'=>$errors];
    }
