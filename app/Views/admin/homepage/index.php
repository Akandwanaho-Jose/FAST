    ...
    <div class="homepage-admin-grid">
        <?php foreach ($quickLinks as $link): ?>
            <form class="content-form homepage-setting-card" method="post" action="<?= View::escape($baseUrl) ?>admin/homepage/quick-links/<?= (int) $link['id'] ?>">
                ...
                <div class="form-field"><label>Collage image</label><select name="media_id" data-media-upload="<?= View::escape($baseUrl) ?>admin/media"><option value="">Use an automatic faculty image</option><?php foreach ($images as $image): ?><option value="<?= (int) $image['id'] ?>" <?= (int) ($link['media_id'] ?? 0) === (int) $image['id'] ? 'selected' : '' ?>><?= View::escape($image['original_name']) ?></option><?php endforeach; ?></select><small>The first three distinct images form the homepage collage.</small></div>
                <div class="form-grid">
